	...
	def collect(self) -> SystemSnapshot:
		...
		if self._previous_time is not None:
			...
			if time_diff <= 0:
				...
				if self._previous_disk is not None and current_disk is not None:
					prev_read, prev_write = self._previous_disk
				...
				if self._previous_net is not None and current_net is not None:
					prev_sent, prev_recv = self._previous_net
...
_default_collector = SnapshotCollector()
